...
'Input Parameters
Const mouseSensitivity As Single = 0.01
...
lbl_wmActivate:
    isNotActive = CLng(wParam / 65536) 'High Word
    GoTo lbl_WndProc_Return0
...
lbl_wmMouseMove:
    ...
    If nMouseMode = 1 Then   ' LMB – rotate model
        Call TrackballRotation(xPrevPos, yPrevPos, xCurPos, yCurPos, RectWidth, RectHeight, tbRot)
        Call CvtQtoM(tbRot)
        Call glMatrixMode(GL_MODELVIEW)      ' ? modelview for object
        Call glMultMatrixf(lp_mtxRotationBuffer)
        isRefreshed = 0
    ElseIf nMouseMode = 2 Then
        ' MButton translation (unchanged)
        Call glMatrixMode(GL_MODELVIEW)
        dStep = dxMouse * (-8)
        Call glTranslatef(dStep * mtxModelViewBuffer(0), dStep * mtxModelViewBuffer(4), dStep * mtxModelViewBuffer(8))
        dStep = dyMouse * 16
        Call glTranslatef(dStep * mtxModelViewBuffer(1), dStep * mtxModelViewBuffer(5), dStep * mtxModelViewBuffer(9))
        isRefreshed = 0
    ElseIf nMouseMode = 3 Then   ' RMB – rotate camera
        Call TrackballRotation(xPrevPos, yPrevPos, xCurPos, yCurPos, RectWidth, RectHeight, tbRot)
...
lbl_wmMouseWheel:
    If CLng(wParam And &H80000000) = 0 Then 'GET_WHEEL_DELTA_WPARAM(wParam) == HIWORD(wParam) == WHEEL_DELTA == either 120 or (-120)
    ...
    Call glTranslatef(dStep * mtxModelViewBuffer(2), dStep * mtxModelViewBuffer(6), dStep * mtxModelViewBuffer(10))
    isRefreshed = 0
    GoTo lbl_WndProc_Return0
...
' Compute trackball rotation quaternion from previous to current mouse position
' Window size (RectWidth, RectHeight) is needed to normalise coordinates
Sub TrackballRotation(ByVal prevX As Long, ByVal prevY As Long, ByVal currX As Long, ByVal currY As Long, _
                      ByVal winW As Long, ByVal winH As Long, ByRef qOut As QUATERNION)
    ...
    ' Compute rotation axis = cross(p1, p2)
    axis.x = p1.y * p2.z - p1.z * p2.y
    axis.y = p1.z * p2.x - p1.x * p2.z
    axis.z = p1.x * p2.y - p1.y * p2.x
    
    ' Angle = dot(p1, p2) -> cos(angle)
    ...
    ' Normalise axis
    norm = Sqr(axis.x * axis.x + axis.y * axis.y + axis.z * axis.z)
    If norm > 0 Then
        axis.x = axis.x / norm
        axis.y = axis.y / norm
        axis.z = axis.z / norm
    Else
        ' No rotation
        qOut.x = 0: qOut.y = 0: qOut.z = 0: qOut.w = 1
        Exit Sub
    End If
    
    ' Build quaternion from axis-angle
    angle = angle * 0.5
    qOut.x = axis.x * Sin(angle)
    qOut.y = axis.y * Sin(angle)
    qOut.z = axis.z * Sin(angle)
    qOut.w = Cos(angle)
End Sub

' Project a 2D screen point onto a 3D unit sphere (trackball)
' Coordinates: x, y in range [-1..1], aspect = width/height
...
'Apply camera rotation (RMB) – this turns the head
    Call CvtQtoM(qCamRot)
    Call glMultMatrixf(lp_mtxRotationBuffer)
'Now apply camera position and base tilt (these are now relative to the rotated view)
